...
| 11 | ゲスト表示 | 下部中央の固定バッジ、`layouts/application.html.erb` に設置、リンクなし |
...
`app/views/layouts/application.html.erb` に下部中央の固定バッジを置く。文言は「ゲスト利用中」のみで、登録へのリンクは置かない。
...
配置が下部中央なのは、左上がハンバーガーボタン（`absolute top-6 left-6 z-60`）、右上がユーザーカードと浄化タイマーカードに占められているため。

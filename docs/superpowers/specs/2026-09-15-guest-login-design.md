...
`app/views/layouts/application.html.erb` に左上のバッジを置く。文言は「ゲスト利用中」のみで、登録へのリンクは置かない。

`_hamburger_menu` は4画面（`mypages` / `mystatuses` / `activity_records#index` / `regret_records#index`）でしか描画されておらず、フォーム系の画面には出ないため、ここには置けない。レイアウトは現状 `flash_message` と `yield` だけの薄い構成なので、追加しても影響が読みやすい。

**当初は下部中央の `fixed` 配置にしたが、実機で確認して取りやめた。** マイページのポモドーロ「スタート」ボタン、後悔記録一覧のカード本文、マイステータスのグラフと、確認したすべての画面で本文に重なった。`pointer-events-none` を付けているのでクリックは貫通するが、見た目が壊れる。どの画面も中央下まで要素が伸びており、下部中央に空きがあるという前提が誤りだった。

採用した配置は左上（`top-7 left-56`）。制約は2つある。

- **フラッシュメッセージへの追随**: `_flash_message` は `fixed` ではなく通常フローのブロックなので、表示されると本文全体が押し下げられる。バッジを `fixed` にするとフラッシュの上に重なる。そこで**高さを持たない `relative` の div をフラッシュの直後に置き、その中で `absolute` に配置する**。これによりハンバーガーボタンと同じだけ下がり、常にその横に並ぶ。
- **左上を占める要素が画面ごとに違う**: ハンバーガーボタン（`left-6` + `w-10` = 64px まで）に加え、`dark_times/show` / `light_times/show` / `purification_times/show` / `users/registrations/show` / `static_pages/how_to_use` の5画面には「← マイページに戻る」（約200px まで）がある。`left-56`（224px）は両方を避ける。

`z-30` はハンバーガーのオーバーレイ（`z-40`）より下に置いている。メニューを開いたときはオーバーレイの下に隠れる。

登録リンクを置かない理由は 9.1 に記載する。

### 5.10 ログアウトと時間切れの案内

ゲストのログアウトはトップページ（`root_path`）へ戻す。ボタンの文言は「ゲストを終了」とする。

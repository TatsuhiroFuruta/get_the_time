...
未スタート時は「終了する」もモチベーションボタンも隠れるため、このアラートは **通常の操作では到達不能**になる。`hidden` は Tailwind の `display: none` であり、クリックできずタブ順からも外れる。それでも `completeSession()` の先頭にガードとして残す。

**到達しうるのは異常時である。** 現実的な順に、Tailwind のビルド成果物が読み込まれない／404 になって `.hidden` が効かなくなる場合、DevTools やユーザースクリプトで改変された場合、そして将来の変更で表示制御が壊れた場合（Turbo のスナップショット復元で実際に一度起きている。前節を参照）。

ガードを外した場合に起きるのは `TypeError` である。`saveActivityRecord()` の `this.firstStartedAt.toISOString()` が `null` に対して呼ばれて投げ、クリックが無反応になる。**壊れたレコードがサーバへ飛ぶわけではない。**

> 当初この節には「`total_duration` に `NaN` が入る」と書いていたが誤りだった。`null` は算術で `0` に変換されるので `NaN` にはならず（`lastEndedAt - null` は epoch からのミリ秒）、そもそも差分計算の直後の `toISOString()` で例外になるためそこまで到達しない。
>
> したがってガードの価値は当初考えたより低く、「異常時に無反応になる代わりに日本語のメッセージが出る」程度である。それでも 4 行で済み、2 つの入口の合流点で表示制御という離れた場所の正しさに依存せずに済むため、残す判断は変えない。

ただし**通常操作では到達不能である以上、これを UI 経由で検証する System Spec は持たない**。代わりに「未スタート時にモチベーションボタンが表示されないこと」をテストする。
